...
if "CUDA_VISIBLE_DEVICES" not in os.environ:
    os.environ["CUDA_VISIBLE_DEVICES"] = ""   # default to CPU unless explicitly enabled
...
def _derive_epoch_key(key: bytes, epoch: int) -> bytes:
    h = hashlib.sha256()
    h.update(key)
...
def _encrypt_aead(plaintext: bytes, key_str: str, rotation_seconds: int = 0) -> bytes:
    """
    AEAD encryption (AES-GCM).
    If key_str is empty, returns plaintext unchanged (no-encryption mode).
    """
    ...
    if rotation_seconds and rotation_seconds > 0:
        ...
        key = _derive_epoch_key(key, epoch)
...
def _decrypt_aead(
    ...
    If key_str is empty, returns blob unchanged (no-encryption mode).
    """
    key = _derive_key(key_str)
    if key is None:
        return blob
    ...
    for offset in range(grace + 1):
        ...
        if try_epoch < 0:
            continue
        try:
            trial_key = _derive_epoch_key(key, try_epoch)
            return AESGCM(trial_key).decrypt(nonce, ct, None)
        except Exception as exc:
            last_exc = exc
            continue
    raise last_exc or ValueError("AES-GCM decryption failed")
...
def encode_message(
    ...
    if rotation_seconds and rotation_seconds > 0:
        header["key_epoch"] = int(time.time()) // rotation_seconds
    ...
    blob = _encrypt_aead(base_plain, key_str, rotation_seconds=rotation_seconds)
    t2 = time.perf_counter()
    if perf is not None:
        perf["serialize_ms"] = (t1 - t0) * 1000.0
        perf["encrypt_ms"] = (t2 - t1) * 1000.0
        perf["total_ms"] = (t2 - t0) * 1000.0
        perf["header_bytes"] = len(header_bytes)
        perf["tensor_bytes"] = len(tensor_bytes)
        perf["pad_bytes"] = int(pad_len)
        perf["crypto_overhead_bytes"] = len(blob) - len(base_plain)
...
def decode_message(
    ...
    - Decrypts 'blob' with AES-GCM (symmetric).
    - Reads:
    ...
    # Expose nonce to callers when encrypted (helps replay detection).
    if key_str:
        header["nonce"] = blob[:12].hex() if len(blob) >= 12 else None
    else:
        header["nonce"] = None
    if perf is not None:
        pad_len = len(padded_plain) - (4 + header_len + tensor_len)
        perf["decrypt_ms"] = (t1 - t0) * 1000.0
        perf["deserialize_ms"] = (t2 - t1) * 1000.0
        perf["total_ms"] = (t2 - t0) * 1000.0
        perf["header_bytes"] = int(header_len)
        perf["tensor_bytes"] = int(tensor_len)
        perf["pad_bytes"] = int(max(0, pad_len))
        perf["crypto_overhead_bytes"] = len(blob) - len(padded_plain)
        perf["total_bytes"] = len(blob)
        header["_perf"] = perf
...
def _normalize_dataset_name(dataset: str) -> str:
    ...
    name = name.replace("_", "-")
    if name in ("fashion", "fashionmnist"):
        return "fashion-mnist"
    return name


def load_dataset(dataset: str = "mnist", normalize: bool = True):
    """Loads a supported dataset and adds a channel dimension."""
    name = _normalize_dataset_name(dataset)
    if name == "mnist":
        ...
        raise ValueError(
            f"Unsupported dataset '{dataset}'. Use 'mnist' or 'fashion-mnist'."
        )
...
def load_mnist(normalize: bool = True):
    """Backward-compatible MNIST loader."""
    return load_dataset("mnist", normalize=normalize)
...
def main(config_path: str = "config.yaml"):
    ...
    if architecture == "single-blind-pool":
        from clients.pool_client import main as pool_main
        return pool_main(config_path)
    ...
    if architecture == "federated":
        from clients.federated_client import main as federated_main
        return federated_main(config_path)
    raise ValueError(
        f"Unknown architecture '{architecture}'. Supported: board-blind, vanilla-split, single-blind-pool, single-blind-bucket, double-blind, federated."
    )
