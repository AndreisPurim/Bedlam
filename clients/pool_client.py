#!/usr/bin/env python3
"""
pool_client.py -- single-blind pool mode using Board audience pools.

Flow:
 - M1M3 posts encrypted requests to audience "to_m2".
 - M2 peers poll the pool, decrypt, filter by target_m2 inside header, process, and
   respond to audience "to_clients".
 - M1M3 polls the pool for responses, decrypts, matches by session/op, and acks.
"""
...
from peers.base import BasePoolBoardClient, BasePoolM1M3Peer, BasePoolM2Peer, BasePIRBoardClient, stratified_split
...
@ray.remote
class PoolPeerM2(BasePoolM2Peer):
    """M2 holder in pool mode."""
    ...
    def __init__(
        ...
        audience_in: str = AUDIENCE_TO_M2,
        audience_out: str = AUDIENCE_TO_CLIENTS,
        use_pir: bool = False,
        ...
        board_client = BasePoolBoardClient(board_host, board_port, logger=logger)
        ...
        logger.info(
            "Initialized PoolPeerM2 input_dim=%d lr=%.4f verbose=%s log_every=%d shared_key_len=%d board=%s:%d",
            input_dim,
            lr,
            self.verbose,
            self.log_every,
...
@ray.remote
class PoolPeerM1M3(BasePoolM1M3Peer):
    """M1+M3 client using encrypted envelopes and pool routing."""
    ...
    def __init__(
        ...
        audience_in: str = AUDIENCE_TO_M2,
        audience_out: str = AUDIENCE_TO_CLIENTS,
        pseudonym_scope: str = "per_session",
        poll_shuffle: bool = False,
        hide_sender: bool = False,
        use_pir: bool = False,
        pir_chunk_size: int = 32,
        ...
        logger.info(
            "Initialized PoolPeerM1M3 name=%s pseudonym=%s epochs=%d batch_size=%d lr=%.4f target_m2=%s",
            self.actor_name,
            self.pseudonym,
            epochs,
            batch_size,
    ...
    def evaluate(self) -> float:
        ...
        self.logger.info(
            "Summary name=%s sender_id=%s target_m2=%s pseudonym_scope=%s hide_sender=%s bytes_sent=%d bytes_received=%d acc=%.4f",
            self.actor_name,
            self.sender_id,
            self.target_m2,
...
def main(config_path: str = "config.yaml"):
    ...
    audience_in = general.get("pool_audience_m2", AUDIENCE_TO_M2)
    audience_out = general.get("pool_audience_clients", AUDIENCE_TO_CLIENTS)
    ...
    global_logger.info(
        ...
        f"architecture=single-blind-pool, model_architecture={model_arch}, dataset={dataset}"
    )
    ...
    global_logger.info(f"Using pool Board at {board_host}:{board_port}")
    ...
    for m2_cfg in m2_peers_cfg:
        ...
        global_logger.info(f"Spawned Pool M2 peer: {name} (key_len={len(key)})")

    # Launch M2 processing loops
    for name, m2_peer in m2_peers.items():
        m2_peer.run.remote()
        global_logger.info(f"Started run() loop for Pool M2 peer: {name}")
    ...
    for i, c_cfg in enumerate(m1m3_peers_cfg):
        ...
        global_logger.info(
            f"Spawned Pool M1M3 peer: {name} -> bound to M2 peer: {target_m2} (key_len={len(key)})"
        )

    # Train all clients
    global_logger.info("Starting training for all pool-mode clients...")
    ray.get([c.train.remote() for c in clients])

    # Evaluate each client
    global_logger.info("Evaluating clients on test set (pool mode)...")
