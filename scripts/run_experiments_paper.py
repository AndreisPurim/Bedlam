...
def prepare_config(
    ...
    cfg["general"]["use_pir"] = arch == "double-blind"
...
def run_one(
    ...
    run_name = f"paper_{arch}_{dataset}_{model}_p{num_m1m3}m2_{num_m2}_{int(time.time())}"
...
def main(args):
    ...
    parent_name = args.paper_name or f"paper_run_{int(time.time())}"
    ...
    print(f"[paper-run] {parent_run_dir}", flush=True)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Minimal paper runs (fast).")
