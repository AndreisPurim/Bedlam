...
def prepare_config(
    ...
    if arch == "federated" and num_m2 < 1:
        num_m2 = 1
...
def run_combo(
    ...
    if arch == "federated" and num_m1m3 < 1:
        print("[skip] federated requires at least 1 client", flush=True)
        return
        ...
        if combo_proc.returncode not in (0, None):
            ...
            try:
                subprocess.run(
                    [
                        sys.executable,
                        str(REPO_ROOT / "scripts" / "telemetry_summary.py"),
                        "--run_dir",
                        str(REPO_ROOT / "runs" / run_name),
                        "--output",
                        str(SUMMARY_CSV),
                        "--append",
                    ],
