#!/usr/bin/env python3
"""
Fast paper results (Option 1):
- vanilla-split (mnist)
- board-blind  (mnist)
- federated    (mnist)

Tiny, quick runs for plotting when double-blind/PIR is unstable.
"""
...
def run_one(
    ...
    cfg = prepare_config(base_cfg, run_name, arch, dataset, model, pad, num_m1m3, num_m2, epochs, parent_run_dir)
...
def main(args):
    ...
    print(f"[paper-fast-1] {parent_run_dir}", flush=True)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fast paper runs (option 1).")
